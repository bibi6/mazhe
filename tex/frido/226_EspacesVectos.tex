...
\begin{enumerate}
	\item
	      \( Q(E)\) sont les formes quadratiques réelles sur \( E\), définition \ref{DefBSIoouvuKR}.
	\item
	      \( S(n,\eR)\) sont les matrice symétriques définition \ref{NORMooAJLHooQhwpvr}.
	\item
	      \( \gS(E)\) est l'ensemble des opérateurs autoadjoints (définition \ref{DEFooYAWTooGUpAvw}) de \( E\) quand \( E\) est un espace euclidien réel.
\end{enumerate}
...
\begin{lemma}       \label{LEMooWRCIooWkMpoF}
	...
	\begin{equation}  \label{EQooJETQooIjxRWu}
		...
		\end{aligned}
	\end{equation}
	est une norme.  À droite, nous trouvons le produit scalaire sur \( E\) et la valeur absolue usuelle sur \( \eR\).
...
\begin{proof}
	...
	\begin{subproof}
		...
		\spitem[Pour \( \gS(E)\)]
		%-----------------------------------------------------------
		Si \( N(A)=0\), alors pour tout \( x\) nous avons \( | x\cdot A(x) |=0\).
		%TODOooXQKZooEuzxGX. Finir la preuve.
	\end{subproof}
\end{proof}
