...
\begin{proposition}		\label{PropBoitPtLoin}
	Soient $E$ un espace vectoriel topologique, $a$ dans $E$ et $x$ tel que $d(a,x)=r$, c'est à dire $x\in S(a,r)$. Dans ce cas, toute boule centrée en $x$ contient un point $P$ tel que $d(P,a)>r$ et un point $Q$ tel que $d(Q,a)<r$.
\end{proposition}
...
\begin{proof}
   ...
   Plus précisément, nous considérons le point
	...
	\begin{equation}
		d(P,x)=d (\frac {x-a}{ N }, 0)
	\end{equation}
	peut être rendu aussi petit\quext{OK quand c'est normé: le $N$ sort\dots{} mais quand c'est ``juste'' topologique?} que l'on veut par un choix approprié de $N$. Montrons maintenant que $d(a,P)>d(a,x)$ :
	\begin{equation}
		\begin{aligned}[]
			d(a,P)&=d (a - x, \frac {x-a}{ N }) \\
			&=d (0, (x-a) ( 1 + \frac 1 N ))\\
			&>d(a,x).
		\end{aligned}
	\end{equation}
