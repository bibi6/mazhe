...
On y va peut-être un peu fort en définissant encore une nouvelle structure, mais elle nous permettra de faire plein de choses ensuite.
...
%--------------------------------------------------------------------------------------------------------------------------- 
\subsection{Définitions et morphismes}
%---------------------------------------------------------------------------------------------------------------------------
...
\begin{definition}
  Soit \( M \) et \( N \) deux modules sur un même anneau \( A \). Un \defe{morphisme}{morphisme!de modules} (ou homomorphisme) de modules est une application de \( M \) dans \( N \) qui respecte la structure de modules. En d'autres termes, une application \( f : M \to N \) est un morphisme de modules si:
  \begin{enumerate}
    \item
      \( f \) est un morphisme de groupes additifs entre \( M \) et \( N \);
    \item
      pour tout \( a \in A\), et tout \(m  \in M\), on a \( f(am) = a f(m) \).
  \end{enumerate}
  L'ensemble des morphismes de modules sur \( A \) entre \(M \) et \( N \) est noté \( \Hom_A(M, N)\)
\end{definition}
...
\begin{lemma}
    ...
    \begin{equation}
        e_i=(\delta_{ij})_{j\in I}, \qquad\text{où } \delta_ij = 1 \text{si } i = j \text{ et } 0 \text{ sinon.}
    \end{equation}
    Alors la famille \( \{ e_i \}_{i\in I}\) est libre\footnote{Définition \ref{DEFooRUKVooLnXxdS}.} dans \( A^I\).
\end{lemma}
...
\begin{theorem}[Propriété universelle de \( A^{(I)}\)\cite{BIBooSTWWooItiMUp}]      \label{THOooPDZCooJnHbOd}
    Soient un anneau \( A\) ainsi qu'un \( A\)-module \( P\). Si \( \phi \) est un homomorphisme de \( A \)-modules de \(A^{(I)}\) vers \(P\), nous considérons
    \begin{equation}
        \begin{aligned}
            \phi|_I\colon I&\to P \\
            i&\mapsto \phi(e_i). 
...
Nous allons définir et étudier ici l'anneau des polynômes sur un anneau \( A\), c'est-à-dire ce qui sera noté \( A[X]\). Certes, on l'a déjà construit d'une certaine façon précédemment, mais la définition qui suit est plus formelle. Pour \( \eK(X)\) lorsque \( \eK\) est un corps, voir~\ref{DEFooQPZIooQYiNVh}.

L'ensemble des polynômes sur \( A\) sera simplement \( A^{(\eN)}\), sur lequel nous allons mettre une structure. Il a déjà une structure de \( A\)-module, nous allols lui ajouter celle d'anneau ainsi qu'une évaluation.
...
Nous ne voyons pas encore bien le lien avec la notation \( A[X]\), et pour cause: on n'a pas encore le produit.
...
\begin{definition}      \label{DEFooNXKUooLrGeuh}
    ...
    \begin{description}
        ...
        \end{equation}
        Notons qu'à droite, la somme sur \( k\) est une somme finie (car \(k \) n'ira pas au-delà de \( m + n \) pour des termes non nuls).
    \end{description}
\end{definition}
...
Si \( A\) est un anneau, nous avons déjà défini les polynômes en une indéterminée sur \( A\) comme étant le module \( A^{(\eN)}\) qui est devenu un anneau par la proposition \ref{PROPooGDQCooHziCPH}. Mais quel lien avec le de la définition~\ref{DefAnneauPolyAX}?

Simplement, on envoie le polynôme donné par la suite \( (a_n)_{n\in \eN}\) sur 
\begin{equation}
    \sum_ka_kX^k.
\end{equation}
Par exemple avec \( a=(4,2,8,0, \dots)\) nous avons \( a=8X^2+2X+4\). Il y a alors (au moins) deux façons de comprendre ce que signifie réellement «\( X\)» dans cette écriture.
...
\begin{propositionDef}
  L'anneau \(A^{(\eN)}\), muni de l'addition "composante par composante" (comme \( A \)-module) et du produit défini à l'aide de~\eqref{EQooTNCSooKklisb}, est (isomorphe à) l'anneau de polynômes \( A[X]\).
\end{propositionDef}

Cela n'est pas si évident, car il faut bien s'assurer que "\( e_k=X^k\)".

\begin{proof}
  L'application, on l'a: elle envoie  \( (a_n)_{n\in \eN}\) sur 
\begin{equation}
    \sum_ka_kX^k.
\end{equation}
Elle respecte bien la somme. Reste à voir pour le produit. Mais l'élément d'indice \(n \) se calcule avec \eqref{EQooTNCSooKklisb}, qui se trouve être la même expression (au nom de lettres près) que les \( c_k \) dans la définition \ref{DefAnneauPolyAX}: le produit est bien respecté aussi.
\end{proof}


%///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
\subsubsection{Comment bien écrire un polynôme?}
%///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
\label{SUBSUBSEC_bienEcrireUnPoly}
...
Le gros avantage d'utiliser les modules pour définir les polynômes est ici: les polynômes de plusieurs variables se définissent facilement.

\begin{definition}      \label{DEFooZNHOooCruuwI}
    L'ensemble des \defe{polynômes de \( n\) variables}{polynôme de plusieurs variables} sur l'anneau \( A\) est \( A^{(\eN^n)}\), c'est-à-dire l'ensemble des suites indexées par \( \eN^n\) et dont seulement une quantité finie de coefficients sont non nuls.
...
Avec la définition \ref{DEF_ExtensionAnneau}, cela donne peut-être l'impression de définir deux fois la même chose. Il n'en est rien grâce à la proposition suivante\quext{Et aux histoires de corps de décomposition, tout ça, tout ça. Faudrait quand même expliciter l'isomorphisme.}.
...
\begin{lemma}   \label{LemISPooHIKJBU}
    ...
    \begin{enumerate}
        ...
        \item \label{ItemLTBooAcyMtNiii}
            \( X^n-a^n=(X-a)\sum_{i=0}^{n-1}a^iX^{n-1-i}\).
    \end{enumerate}
\end{lemma}

\begin{proof}
  Nous démontrons uniquement le point~\ref{ItemLTBooAcyMtN}, puisque
  le point~\ref{ItemLTBooAcyMtNiii} a été démontré dans le cadre des anneaux quelconques\footnote{Voir l'égalité
    \eqref{Eqarpurmkbk}, avec "\( a = X,\ b = a\)"}, et le point~\ref{ItemLTBooAcyMtNii} est le point~\ref{ItemLTBooAcyMtNiii} avec \(a = 1\).
    
  Le cas \( n=1\) est évident. Procédons alors par récurrence en considérant un nombre entier impair \( n\), et supposant que \( 1 + X^n = (1+X)P \) :
    \begin{subequations}
        \begin{align}
