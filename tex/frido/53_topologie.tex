...
\begin{proof}
  ...
  \begin{equation}
    \bigcup_{\alpha \in A} \bigcap_{i=1}^{n_a} \mO_{\alpha, i}\quad\text{ et } \bigcup_{\beta \in B} \bigcap_{j=1}^{n_\beta} \mO_{\beta, j}.
  \end{equation}
  Un élément \( x \) est dans l'intersection de ces deux ensembles s'il existe \( \alpha \in A \) et \( \beta \in B \) tels que pour tout \(i \in \{1, \cdots, n_\alpha\} \) et tout \(j \in \{1, \cdots, n_\beta\} \), \( x \in \mO_{\alpha, i}\) t \(x \in \mO_{\beta, j} \). On construit alors l'ensemble \( A \times B \) pour l'union, et \( n_{\alpha, \beta} = n_\alpha + n_\beta\). Enfin, pour \((\alpha, \beta)\), et pour \( k  \in \{1, \cdots, n_{\alpha,\beta}\} \), on pose
  \begin{equation}
    \mO_{(\alpha, \beta), k} =
    \begin{cases}
    ...
    \end{cases}
  \end{equation}
Il s'ensuit que l'intersection est précisément
\begin{equation}
   \bigcup_{\gamma \in A\times B} \bigcap_{k=1}^{n_\gamma} \mO_{\gamma, k}.
\end{equation}
\end{proof}

\begin{remark}
  La famille composée des intersections finies d'éléments de \( \mT_0 \) forme yune base de la topologie engendrée.
\end{remark}

\begin{definition}[Topologie réelle engendrée par les intervalles ouverts] \label{DefTopologieREngendree}
  Pour \( X = \eR \), considérons \( \mT_0 \) l'ensemble constitué de tous les intervalles ouverts (de la forme \( \mathopen] a, b \mathclose[ \), avec \( a, b \in \eR \)). La topologie ainsi engendrée est alors la topologie usuelle sur \( \eR \).
\end{definition}

\begin{remark} \label{RemIntervallesBaseTopoR}
  Les intervalles ci-dessus sont une base de la topologie de \( \eR \).
\end{remark}

\begin{example}[Voisinages de réels]
  Pour qu'un ensemble soit un voisinage de $x \in \eR$, il faut et il suffit qu'il contienne un intervalle ouvert. Ainsi, les ensembles suivants sont des voisinages de $3$ dans $\eR$ :
  \begin{itemize}
  \item
...
\begin{proof}
Soit $\mO$, un ensemble ouvert et $s$, son supremum. Si $s$ était dans $\mO$, on aurait un voisinage de $s$ (et donc un intervalle $\mathopen]a,b\mathclose[$ contenant $s$) et contenu dans $\mO$. On pose $r = \min\{s - a, b - s\}$; alors, le point $s+r/2$ est alors à la fois dans $\mathopen]a,b\mathclose[ \subset \mO$ et plus grand que $s$, ce qui contredit le fait que $s$ soit un supremum de $\mO$.
\end{proof}
...
\begin{remark}
    Petite remarque à propos des frontières. Soit \( X=\mathopen[ 0 , 1 \mathclose]\) muni de la topologie de la distance \( | x-y |\) (définition~\ref{ThoORdLYUu}). Les points \( 0\) et \( 1\) \emph{ne sont pas} dans la frontière de $X$. En effet une boule ouverte autour de \( 1\) est un ensemble de la forme
    \begin{equation}
