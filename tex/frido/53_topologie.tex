...
\begin{remark}
  La famille \( B(\mT_0)\) composée des intersections finies d'éléments de \( \mT_0 \) forme une base de la topologie engendrée \( \tau(\mT_0)\).
\end{remark}
...
\begin{example}[Topologie des corps totalement ordonnés]
  Si \( \eK \) est un corps totalement ordonné\footnote{Définition \ref{DefKCGBooLRNdJf}.}, alors \( \eK \) peut être muni de la topologie engendrée par ses boules ouvertes.
\end{example}
...
\begin{proposition} \label{TopologieR_ord_top}
  Les topologies de \( \eR \) en tant que corps ordonné (à l'aide de \ref{DefKCGBooLRNdJf}) et de \( \eR \) en tant qu'espace topologique (définition \ref{DefTopologieREngendree}) sont les mêmes.
\end{proposition}

\begin{proof}
  Du corps ordonné \( \eR \), on recrée les intervalles, et donc la topologie. Reste à voir le sens inverse: est-ce que, de tous les intervalles, on recrée les boules? La réponse est oui, grâce à la remarque précédente.
\end{proof}
...
\begin{lemma} \label{FrontiereVidePourOuvertsFermes}
...
\begin{proof}
  Si \( x_n\to \ell\) et si \( \epsilon>0\) il existe \( N \) tel que pour tout \( n\geq N\) nous avons \( x_n\in \mathopen ]\ell-\epsilon,\ell+\epsilon\mathclose[\) (parce que c'est un ouvert contenant \( \ell\)). Mais \( \eR\) est totalement ordonné, et par le lemme~\ref{LemInegEtBoules_CorpsTotalementOrdonne}, cette condition est bien \( | x_n-\ell |<\epsilon\).

    Dans l'autre sens, soit \( \mO\) un ouvert contenant \( \ell\). Par définition de la topologie réelle, il existe \( \epsilon > 0 \) tel que \( ]\ell-\epsilon, \ell+\epsilon\mathclose[\subset\mO \). Par la condition \eqref{EqDefLimSuite}, il existe \( N_{\epsilon} \) tel que pour tout \( n\geq N_{\epsilon}\) nous ayons
    \begin{equation}
...
\begin{proof}
  Ce sont juste les propositions~\ref{PROPooNRRIooCPesgO} et \ref{PROPooNRRIooCPesgO2} qui font tout le boulot. Un exercice pour le lecteur est de récrire les preuves de là-bas en utilisant la topologie usuelle!
\end{proof}
...
\begin{definition}  \label{DefIRKNooJJlmiD}
     Soit \( X \) un espace topologique. Nous disons qu'un sous-ensemble \(A\) est \defe{non connexe}{connexité!définition} quand on peut trouver des ouverts \(O_1\) et \(O_2\) disjoints tels que
    \begin{equation}    \label{EqDefnnCon}
        A\subset O_1 \cup O_2,
    \end{equation}
...
\begin{proposition} \label{PropHSjJcIr}
    ...
    \begin{enumerate}
    \item \label{ITEMooNIPZooIDPmEc}
      L'espace \( X\) est connexe.
    \item  \label{ITEMooNIPZooIDPmEd}
    ...
    \item \label{ITEMooNIPZooIDPmEe}
      Les seuls parties de \(X \) ayant une frontière vide sont \( \emptyset \) et \(X \).
    \end{enumerate}
\end{proposition}
...
\begin{proof}
  \begin{subproof}
    \item[\ref{ITEMooNIPZooIDPmEc}$\Leftrightarrow$\ref{ITEMooNIPZooIDPmEf}:]
      L'ensemble \( X \)  est connexe si et seulement si, pour tout choix d'ouverts disjoints \(O_1 \) et \(O_2\) tels que \(X = O_1 \cup O_2 \), on a \( O_1 = \emptyset \) ou bien \( O_2 = \emptyset \). Or:
      \begin{gather}
        O_1 \cap O_2 = \emptyset \Longleftrightarrow O_1^c \cup O_2^c = X;\\
        X = O_1 \cup O_2 \Longleftrightarrow O_1^c \cap O_2^c = \emptyset;\\
        O_1 = \emptyset \Leftrightarrow O_2 = X \Leftrightarrow O_2^c = \emptyset;\\
        \text{et } O_2 = \emptyset \Leftrightarrow O_1 = X
        \Leftrightarrow O_1^c = \emptyset.
    \end{gather}
    L'équivalence entre les deux premiers points est prouvée.

  \item[\ref{ITEMooNIPZooIDPmEc}$\Leftarrow$\ref{ITEMooNIPZooIDPmEf}:]
    Soit \(A \) un ouvert-fermé de \(X\). L'ensemble \(A^c\) est aussi un ouvert-fermé de \(X\), et il est disjoint de \( A\). Si \( X \) est connexe, alors \( A \) est nécessairement trivial (vide, ou bien de complémentaire vide, soit \( A = X \)).

  \item[non \ref{ITEMooNIPZooIDPmEd}$\Leftarrow$non \ref{ITEMooNIPZooIDPmEf}:]
    Supposons qu'il existe \( A, B\) fermés disjoints non-vides tels que \(X = A \cup B\). Considérons \( B^c \): c'est un ouvert. Il est nécessairement inclus à \( A \) puisque c'est une partie disjointe de \(B \). Or, \( A \cap B = \emptyset \) signifie que \( A \subset B^c \). Donc \( B^c = A \) est à la fois ouvert et fermé, non vide, et non égal à \(X \) puisque \( B \) n'était pas vide.

    \item[ \ref{ITEMooNIPZooIDPmEf}$\Leftrightarrow$ \ref{ITEMooNIPZooIDPmEe}]
      C'est une conséquence directe du lemme \ref{FrontiereVidePourOuvertsFermes}.
  \end{subproof}
\end{proof}
...
\begin{proof}
  Supposons \( B \) recouvert par deux ouverts disjoints. Alors ces deux ouverts recouvrent \(A \). Comme \( A \) est connexe, l'un des ouverts est disjoint de \(A \): cet ouvert est donc dans \( A^c \), donc plus précisément dans \( \Int(A^c) = (\bar A)^c \) par la définition \ref{DEFooSVWMooLpAVZRInt} et \eqref{PropCompleIntBar}, donc est disjoint de \( \bar A \) et en particuiler, disjoint de \(B \).
\end{proof}
...
\begin{proof}
    Nommons \( \gamma\) la partie connexe qui intersecte \( A\) et \( A^c\). Les ouverts \( \mathring A\) et \( X\setminus \bar A\) ne peuvent pas recouvrir \( \gamma\) parce que ce sont deux ouverts disjoints alors que \( \gamma\) est connexe\footnote{Définition~\ref{DefIRKNooJJlmiD}.}. Donc \( \gamma\) doit contenir des points qui sont dans \( \bar A\) mais pas dans \( \mathring A\), c'est à dire des points de \( \partial A\).
\end{proof}
...
\begin{proof}
    La preuve est en deux parties, et en fait, on va faire le coup de \href{http://fr.wikipedia.org/wiki/Contraposée}{la contraposée}. D'abord nous démontrons que si un sous-ensemble de $\eR$ n'est pas un intervalle, alors il ne peut pas être connexe; et ensuite nous démontrons que tout sous-ensemble non connexe est autre chose qu'un intervalle.

    Prenons d'abord $A$, une partie de $\eR$ qui n'est pas un intervalle. Il existe donc $a$, $b\in A$ et un $x_0$ entre $a$ et $b$ qui n'est pas dans $A$. Comme le but est de prouver que $A$ n'est pas connexe, il faut couper $A$ en deux ouverts disjoints. L'élément $x_0$ qui n'est pas dans $A$ est le bon candidat pour effectuer cette coupure. Définissons
    \begin{align*}
        \mO_1&=]\inf A ,x_0[\\
        \mO_2&=]x_0, \sup A[
    \end{align*}
    (avec éventuellement \( \inf A = - \infty, \sup A = + \infty \)). Ce sont deux ensembles ouverts, non-vides (car \( a \in \mO_1 \) et \( b \in \mO_2 \)), disjoints, et leur union recouvre tout $A$. En effet, $\mO_1\cup \mO_2$ contient tous les nombres entre  $\inf A$ et $\sup A$, à l'exception de $x_0$. Mais on sait que $x_0$ n'est pas dans $A$: donc, $A$ n'est pas connexe.

    Réciproquement, prenons une partie $A$ de $\eR$, supposons qu'elle n'est pas connexe et prouvons qu'elle n'est alors pas un intervalle. Nous avons deux ouverts disjoints $\mO_1$ et $\mO_2$ tels que $A\subset \mO_1\cup \mO_2$. Notons \( A_1 = A \cap \mO_1 \) et  \( A_2 = A \cap \mO_2 \); ils sont non-vides par non-connexité de $A$: prenons $a\in A_1$ et $b\in A_2$. Pour fixer les idées, on suppose que $a<b$. Maintenant, le jeu est de montrer qu'il existe un point $x_0$ entre $a$ et $b$ qui ne soit pas dans $A$ (cela montrerait que $A$ n'est pas un intervalle). Nous allons prouver que c'est le cas du point
    \begin{equation}
      x_0=\sup \mA \qquad \text{avec }\mA = \{ x\in\mO_1\tq x<b \}.
    \end{equation}
    Étant donné que $\mA$ est ouvert (c'est l'intersection entre l'ouvert $\mO_1$ et l'ouvert $\mathopen] -\infty, b \mathclose[$), le point $x_0$ n'est pas dans $\mA$ par le lemme~\ref{LemSupOuvPas}. Nous avons donc
    \begin{itemize}
        \item soit $x_0$ n'est pas dans $\mO_1$,
        \item soit $x_0\geq b$,
        \item soit les deux en même temps.
    \end{itemize}
    Nous allons montrer qu'un tel $x_0$ ne peut pas être dans $A$. D'abord, remarquons que $\sup\mA\leq\sup\mO_1$ parce que $\mA$ est une intersection de $\mO_1$ avec quelque chose (voir la proposition \ref{PROPooANIOooIJHelX}). Ensuite, il n'est pas possible que $x_0$ soit dans $\mO_2$ : en effet, tout élément de $\mO_2$ possède un voisinage $V$ contenu dans $\mO_2$; Comme $O_1 \cap V \subset O_1 \cap O_2 = \emptyset$, un point de $\mO_2$ est toujours strictement plus grand que le supremum de $\mO_1$, qui lui-même est plus grand que $x_0$.

    Maintenant, remarquons que si $x_0\geq b$, alors $x_0=b$, sinon $b$ serait un majorant de $\mA$ plus petit que $x_0$, ce qui n'est pas possible vu que $x_0$ est le supremum de $\mA$ et donc le plus petit majorant. Oui mais si $x_0=b$, c'est que $x_0\in\mO_2$, ce qu'on vient de montrer être impossible. Nous voilà déjà débarrassé des deuxièmes et troisièmes possibilités.
...
\begin{definition}[Séquentiellement compact] \label{DefEspaceSeqCompact}
...
\begin{proposition}\label{ThoBWFTXAZNHoo}
    Un espace topologique compact et séparé est séquentiellement compact.
\end{proposition}

\begin{proof}
   Soient \( X\) un espace compact et \( (x_n)\) une suite dans \( X\). Nous considérons la suite de fermés emboîtés
   \begin{equation}
...
\begin{remark}[Séquentiellement compact n'est pas nécessairement compact \footnote{\url{http://www.les-mathematiques.net/phorum/read.php?14,359450}}] \label{ContreExSeqCompactNonCompact}
  Un espace séquentiellement compact n'est pas nécessairement compact. Voici un contre-exemple, basé sur les ordinaux.
  \begin{probleme}
    Faire les ordinaux.
  \end{probleme}

  On considère \( \omega_1 \) le premier ordinal non-dénombrable, et les ensembles de la forme \( \{ x \tq x \in y \} \) pour \( y \in \omega_1\). Ces ensembles forment une topologie sur \( \omega_1 \).

De tous les ensembles ainsi construits, on ne peut pas obtenir un recouvrement de \( \omega_1 \) fini: s'il en existait un, on aurait un \( y \in \omega_1\) maximal, mais \( y+1 \in \omega_1\) n'est pas recouvert. L'ensemble ainsi construit n'est pas compact.

En revanche, il est séquentiellement compact. Si \((x_n)\) est une suite, il faut sortir une sous-suite qui converge. L'ensemble \( \{x_n \tq n \in \eN \}\) possède un plus petit élément indicé par \( n_0 \). L'ensemble \( \{x_n \tq n \in \eN \text{ et } n > n_0\}\) admet un plus petit élément, indicé par \( n_1 \). Pour un entier naturel \( k \), une fois \(n_k\) fixé, l'ensemble \( \{x_n \tq n \in \eN \text{ et } n > n_k\}\) admet un plus petit élément, indicé par \( n_{k+1} \). Alors, la suite \( (x_{n_k})_{k \in \eN} \) est une sous-suite de \( (x_n)\), croissante pour l'inclusion, et elle converge vers \( \cup_{k \in \eN} x_{n_k}\).
\end{remark}
...
Soit $f\colon A\subset\eR^n\to B\subset\eR^m$ une bijection continue. Si $A$ est compact, alors l'application réciproque \footnote{Définition \ref{DEFooTRGYooRxORpY}.} $f^{-1}\colon B\to A$ est continue.
\end{proposition}
