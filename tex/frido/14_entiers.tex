...
\begin{proof}
    On prouve d'abord l'unicité, puis l'existence.
    \begin{subproof}
        \item[Unicité]
            ...
            \end{equation}
            Donc \( q\) est unique et la relation \( a=bq+r\) implique que \( r = a-qb\) est également unique.
            \item[Existence]
...
\begin{definition}      \label{DEFooWYIGooRVBTil}
    On note \( \Delta_n \) l'ensemble \( \Delta_n = \{ k \in \eN \tq k < n \text{ et } \pgcd(k,n)=1\} \).La fonction \( \varphi\) donnée par
...
\end{definition}
La fonction indicatrice d'Euler a quelques propriétés intéressantes, que nous développerons un peu en section~\ref{SEC_propIndicatriceEuler}. Remarquons néanmoins que si \( p\) est premier, nous avons \( \varphi(p)=p-1\) parce que tous les entiers de \( \{ 1,\ldots, p-1 \}\) sont premiers avec \( p\).
...
\begin{proposition}     \label{PropZpintssiprempUzn}
    Soient \( n\geq 2\) un entier et \( \pi\colon \eZ\to \eZ/n\eZ\) la surjection canonique. Alors l'ensemble des inversibles de \( \eZ/n\eZ\) est donné par
    \begin{equation}
        U(\eZ/n\eZ)=\pi(\Delta_n)=\{ [x]_n \tq 0\leq x < n\text{ et }\pgcd(x,n)=1 \}.
    \end{equation}
...
\begin{proof}
    ...
    \begin{equation}
        [u][x] \tilde [1],
    \end{equation}
    donc \( [u] \) est l'inverse de \( [x]\). Cela prouve que \( \pi(\Delta_n)\subset U(\eZ/n\eZ)\).

    Nous prouvons maintenant l'inclusion inverse. Soient \( [x] \) et \( [y] \) inverses l'un de l'autre : \( [x][y]=[1] \). Or, \([x][y] = [xy]\), donc \( xy \) et \( 1 \) définissent la même classe. Il existe donc \( q\in\eZ\) tel que \( xy-qn=1\), ce qui prouve\footnote{À nouveau avec le Théorème de Bézout.} que \( \pgcd(x,n)=1\).
\end{proof}
...
\begin{proposition}[\cite{BIBooFOGTooQgFAbQ}]       \label{PROPooMTWGooEMbvDi}
    ...
    \end{enumerate}
    De plus, pour \( n \) fixé, il y a exactement \( \varphi(n) \) nombres \( m \) qui vérifient la propriété.
\end{proposition}

\begin{proposition}[\cite{BIBooFOGTooQgFAbQ}]       \label{PROPooSKSYooZoDhIP}
    Soit \( n\geq 2\). Le groupe \( \eZ/n\eZ \) est exactement décrit par les ensembles suivants:
    \begin{enumerate}
        \item
            \( \eZ/n\eZ=\{ [b+k]_n \tq k=0,\ldots, n-1\}\).
        \item
            Pour \( a \) premier avec \( n\), \( \eZ/n\eZ=\{ [ka]_n \tq k=0,\ldots, n-1\} \}\).       
        \item
            Pour \( a \) premier avec \( n\), \( b\in \eN\), et \( m_1,\ldots, m_n\in \eN\), si \( \eZ/n\eZ=\{ [m_k]_n \tq k=1,\ldots, n\} \), alors \( \eZ/n\eZ=\{ [am_k+b]_n \tq k=1,\ldots, n\} \)
    \end{enumerate}
\end{proposition}

\begin{proof}
  Nous explicitons les fonctions de \( \eZ/n\eZ=\{ [k]_n \tq k=0,\ldots, n-1\}\) avec les autres ensembles, et prouvons ensuite que ce sont des bijections.
  \begin{enumerate}
    \item
      \( [k]_n \mapsto [b+k]_n \)
    \item
      \( [k]_n \mapsto [ak]_n \)
    \item
      \( [m_k]_n \mapsto [am_k+b]_n \)
  \end{enumerate}
  La première est clairement une surjection: la classe \( [x]_n \) est atteinte, pour tout \( x \), par \( [x - b]_n \).
  
  Pour la deuxième, comme \( a \) premier avec \( n\), \( [a]_n\) est inversible dans \( \big( (\eZ/n\eZ)^*,\cdot \big)\) \footnote{Proposition~\ref{PROPooMTWGooEMbvDi}}; soit \( [z]_n \) tel que \( [a]_n[z]_n = [1]_n \). La classe  \( [x]_n \) est alors atteinte, pour tout \( x \), par \( [ux]_n \).
  
  La dernière application n'est que la composée des précédentes. Finalement, toutes trois sont des surjections sur des ensembles contenant le même nombre d'éléments: ce sont donc des bijections.\quext{C'est évident a priori, mais il faudrait l'écrire dans le chapitre 1.}
\end{proof}

Petite remarque: dans~\cite{BIBooFOGTooQgFAbQ}, la proposition précédente est démontrée à coup de divisibilité et de lemme de Gauss~\ref{LemPRuUrsD}.


\begin{corollary}   \label{CorZnInternprem}
    L'anneau \( \eZ/p\eZ\) est intègre si et seulement si \( p\) est premier.
\end{corollary}
...
\begin{proposition}[\cite{BIBooYOLUooOAUbYH}]       \label{PROPooKSCRooPyInSv}
    Le groupe \( \big( (\eZ/p\eZ)^*, \cdot\big)\) est cyclique si \( p \) est premier.
\end{proposition}

Keith Conrad, dans \cite{BIBooYOLUooOAUbYH}, a synthétisé \emph{sept} preuves de ce résultat! Contentons-nous d'une seule\dots

\begin{proof}
  Il suffit de démontrer qu'il y a un élément d'ordre \( p - 1 \) dans \( (\eZ/p\eZ)^* \) (qui contient \( p-1 \) éléments). On note \( n \) l'ordre maximal parmi les éléments de \( (\eZ/p\eZ)^* \); alors, par le corollaire du théorème de Lagrange~\ref{CorpZItFX}, \(n \) divise \( p - 1 \), donc plus simplement \( n \leq p - 1 \). Par ailleurs, tout élément \( x \) de \( (\eZ/p\eZ)^* \) satisfait \( x^n = 1 \), donc est une racine du polynôme \( X^n - 1 \) dans l'anneau \( \eZ/p\eZ \), intègre par le corollaire~\ref{CorZnInternprem}. Or, le théorème~\ref{THM_racinesPoly} nous indique que ce polynôme n'a que \( n \) racines au maximum: on en déduit \( p - 1 \leq n \). Conclusion: l'ordre maximal d'un élément est \( n = p - 1 \) dans \( (\eZ/p\eZ)^* \), c'est donc un groupe cyclique.
\end{proof}
...
%---------------------------------------------------------------------------------------------------------------------------
\subsection{Facteurs premiers et indicatrice d'Euler}
%---------------------------------------------------------------------------------------------------------------------------
\label{SEC_propIndicatriceEuler}
...
\begin{proof}
    La première égalité est évidente: tous les entiers positifs inférieurs à \( p \) sont premiers avec \( p \). Pour la seconde, notons que les éléments de \( \{ 1,\ldots,p^n \}\) qui ont un \( \pgcd\) différent de \( 1\) avec \( p^n\) sont tous les nombres qui s'écrivent sous la forme \( qp\) avec \( q\leq p^{n-1}\)\footnote{Corolaire~\ref{CORooQIMHooUzLUJY}.}. Il y a évidemment \( p^{n-1}\) tels nombres.
...
\begin{corollary}       \label{CorlvTmsf}
    L'indicatrice d'Euler est multiplicative : si \( p\) est premier avec \( q\), alors \( \varphi(pq)=\varphi(p)\varphi(q)\). De plus si \( p\) et \( q\) sont deux nombres premiers,
    \begin{equation}
        \varphi(pq)=(p-1)(q-1).
    \end{equation}
\end{corollary}
