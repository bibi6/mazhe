...
\begin{lemma}[\cite{MonCerveau}]       \label{LemooWCZMooZqyaHd}
	...
	\begin{enumerate}
		...
		\item	\label{ITEMooUTDXooFNgHqo}
		      Si la dimension de \( E\) est paire, alors le polynôme caractéristique \( \chi_u\) est unitaire.
	\end{enumerate}
\end{lemma}

\begin{proof}
	Nous considérons une base \( \{ e_i \}_{i=1,\ldots,n}\) de \( E\). En utilisant la définition \ref{LEMooQTRVooAKzucd} donne
	\begin{subequations}
		\begin{align}
			\chi_u(X) & =\det(u-X\mtu_n)                                                                                                       \\
			          & =\sum_{\sigma\in S_n}\epsilon(\sigma)\prod_{i=1}^ne_{\sigma(i)}^*\big( (u-X)(e_i) \big)                                \\
			          & =\sum_{\sigma\in S_n}\epsilon(\sigma)\prod_{i=1}^n\Big( e^*_{\sigma(i)}\big( u(e_i) \big)-Xe^*_{\sigma(i)}(e_i) \Big).
		\end{align}
	\end{subequations}
	Par la proposition \ref{PROPooUIZOooCqQzJh}, le terme de plus haut degré dans \( \chi_u(X)\) serait
	\begin{equation}
		\sum_{\sigma\in S_n}\epsilon(\sigma)\prod_{i=1}^{n}(-)Xe^*_{\sigma(i)}(e_i)=(-1)^nX^n\sum_{\sigma\in S_n}\prod_{i=1}^n\delta_{i,\sigma(i)}.
	\end{equation}
	si ce n'est pas nul. Si \( \sigma\neq \id\), alors il y a au moins un des \( \delta_{i,\sigma(i)}\) qui est nul, et donc tout le produit est nul. Il reste donc de la somme seulement le terme \( \sigma=\id\). Bref, le terme de plus haut degré dans \( \chi_u(X)\) est
	\begin{equation}
		(-1)^nX^n.
	\end{equation}
	Cela prouve \ref{ITEMooAALOooGlQYOs}. Quant à \ref{ITEMooUTDXooFNgHqo}, c'est également prouvé parce que si \( n\) est pair, alors \( (-1)^n=1\).
\end{proof}
