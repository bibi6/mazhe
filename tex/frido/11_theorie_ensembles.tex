...
\begin{proof}
    D'abord nous avons l'égalité \( X^c\setminus Y^c=Y\setminus X\). Cela se prouve en étudiant selon que \( Y\) soit inclus dans \( X\) ou non. S'il existe un élément dans \(Y\setminus X\), alors:
    \begin{itemize}
     \item il est dans \(Y \) mais pas dans \(X \),
     \item donc il est dans \(X^c \) et il n'est pas dans \(Y^c\),
     \item donc il est dans \( X^c\setminus Y^c \).
    \end{itemize}
    S'il n'existe pas de tel élément, alors:
    \begin{itemize}
    \item tous les éléments de \(Y \) sont dans \(X \),
    \item donc tout élément de \(X^c \) est dans \(Y^c \),
    \item et il s'ensuit que \( X^c\setminus Y^c \) ne contient aucun élément non plus.
    \end{itemize}
...
\begin{lemma}       \label{LEMooYMRJooYIAhBb} \label{LEMooFHEOooSHPGgU}
...
\begin{proof}
    ...
    Soit \( x_0\in E_0\setminus E_1\) \footnote{Axiome du choix et tout ça.}. Nous définissons
    \begin{equation}
        \begin{aligned}
