...
\begin{proof}
Ce qu'il faut prouver, c'est que la définition par l'équation~\eqref{EQDEF_detPermutations} donne bien la même chose que la définition de la sous-section~\ref{SUBSUBSEC_calculDeterminantMat}.\quext{Et ça marche, mais faut le faire.}
\end{proof}
...
\begin{proof}
    ...
    \end{equation}
    %TODO ooHLQLooWXznGe Mettre une référence vers la proposition qu'il faut à ce «par conséquent».
...
%TODO : lorsqu'on aura démontré que \eC est algébriquement clos, il faudra le référentier ici.
    ...
    \begin{equation}
        b_k\in\big\{    -\binom{ n }{ n-k },-\binom{ n }{ n-k }+1,\cdots, 0,\cdots,\binom{ n }{ n-k }   \big\},
    \end{equation}
    qui est un ensemble de cardinal \( 2 \binom{ n }{ n-k }+1\). Nous avons donc
    \begin{equation}
        \Card(K_n)\leq\prod_{k=0}^{n-1}\bigg( 1+2 \binom{ n }{ n-k } \bigg)<\infty.
    \end{equation}
...
%Le seul problème est de trouver les trois vecteurs $f_i$. Le premier est tout trouvé : c'est n'importe quel vecteur sur la droite. Pour les deux autres, il faut un peu ruser parce qu'il faut impérativement qu'ils soient perpendiculaires à la droite. Pour trouver $f_2$, on peut écrire
%=======
donc
