...
%---------------------------------------------------------------------------------------------------------------------------
\subsection{Définitions, et lien avec les anneaux}
%---------------------------------------------------------------------------------------------------------------------------

\begin{definition}[\cite{ooLKFGooTUrnhx}]  \label{DefTMNooKXHUd}
    Un \defe{corps}{corps} est un anneau\footnote{Définition \ref{DefHXJUooKoovob}.} dans lequel tout élément non nul est inversible pour l'opération \( \times\) \footnote{Pour l'opération \( +\), tous les éléments sont déjà inversibles parce que \( (A,+)\) est un groupe.}.
\end{definition}
...
\begin{lemma}       \label{LemAnnCorpsnonInterdivzer}  \label{EXooMXNTooZaRPPi}
...
\end{proof}
Une belle conséquence de cela est donnée par la proposition~\ref{DEFooTAOPooWDPYmd}: dans un corps, nous avons toujours la règle du produit nul, et l'élément nul n'est jamais inversible.
...
\begin{definition}[\cite{ooGSDHooLgtHCb}]       \label{DEFooGJYXooOiJQvP}
    Soit \( A \) un anneau commutatif et intègre\footnote{Définition~\ref{DEFooTAOPooWDPYmd}.} Le \defe{corps des fractions}{corps!des fractions} de \( A\) est le localisé de \( A \) en \( A \setminus \{ 0 \} \); en d'autres termes, c'est le quotient
    \begin{equation}
...
\begin{proposition}[\cite{MonCerveau}]      \label{PROPooGSHDooJOnDsp}
    Soit \( A \) un anneau commutatif et intègre. Si \( \eL\) est un corps contenant \( A\) en tant que sous-anneau\footnote{Bien entendu, nous pouvons trouver plein de corps contenant \( A\) en tant que sous-ensemble sans pour autant étendre \( A\) en tant qu'anneau; il suffit d'y mettre une loi de composition farfelue.}, alors il existe un morphisme de corps injectif \( \epsilon\colon \Frac(A)\to \eL\).
\end{proposition}
...
\begin{proof}
    ...
    \begin{equation}
        \epsilon\big( a/b+c/d \big)=\epsilon\big( (ad+cb)/bd)\big)=(ad+cb)(bd)^{-1}=add^{-1}b^{-1}+cbd^{-1}b^{-1} = ab^{-1}+cd^{-1},
    \end{equation}
    (la dernière égalité a lieu par commutativité de \(A \)) tandis que
    \begin{equation}
        \epsilon(a,b)+\epsilon(c,d)=ab^{-1}+cd^{-1},
    \end{equation}
    qui est égal. On laisse le lecteur vérifier que ça marche aussi pour le produit.
...
\begin{definition}[Évaluation d'une fraction rationnelle]       \label{DEFooLBIWooCPCaSY}
    Soit un corps \( \eK\) contenant l'anneau commutatif et intègre \( A\). Si \( R=P/Q\in \Frac(A)[X] \) et si \( \alpha\in \eK\) nous définissons
    \begin{equation}
...
Notons que si \( A\) est un anneau qui n'est pas un corps, le corps \( \Frac(A)\) existe, mais si \( R\in\Frac(A)[X]\), il n'a pas de sens de vouloir calculer \( R(\alpha)\) pour \( \alpha\in A\) "général", parce qu'on a besoin d'inverses.

\begin{theorem}     \label{ThogbhWgo}
    Soit \( A\) un anneau commutatif intègre, et \( \eK\) un corps commutatif. Soit un morphisme d'anneaux \( \phi\colon A\to \eK\) tels que pour tout \( \lambda\in\eK\), il existe \( (a,b)\in A\times A^*\) tels que
    \begin{equation}
        \lambda=\phi(a)\big( \phi(b) \big)^{-1}.
    \end{equation}
    Alors \( \eK\) est isomorphe à \( \Frac(A) \).
\end{theorem}
%TODO: preuve. Dans les grandes lignes:
% - K contient \phi(A), qui doit être un sous-anneau de K.
% - Par la prop précédente, on a un \epsilon: Frac(\phi(A)) \to K, injectif.
% - Sans doute qu'on prolonge \phi telle que \phi: \Frac(A) \to \Frac(\phi(A)), et que la composée \epsilon \circ \phi est la bijection.


\begin{lemma}[Simplification de fraction]
    L'application \( \eA\times \eA^*\to \eK\) donnée par \( (a,b)\mapsto \phi(a)\big( \phi(b) \big)^{-1}\) envoie \( (xa,xb)\) sur le même que \( (a,b)\).
\end{lemma}
...
\begin{proposition}
    Soit un anneau commutatif et intègre \( A\). Tout corps contenant un sous-anneau isomorphe à \( A\) contient un sous-corps isomorphe à \( \Frac(A)\).
\end{proposition}
...
\begin{definition}
    Le corps des fractions de \( \eZ\) (définition~\ref{DEFooGJYXooOiJQvP}) est noté \( \eQ\) et ses éléments sont les \defe{rationnels}{rationnels}. Un nombre \defe{irrationnel}{irrationnel} est un nombre qui n'est pas dans \( \eQ \).
\end{definition}
...
Nous verrons dans la proposition~\ref{PropooRJMSooPrdeJb} que \( \sqrt{2}\) est un nombre irrationnel (qui n'est donc pas dans \( \eQ \). En fait le théorème suivant va nous montrer que le nombre \( \sqrt{ n }\) est soit entier, soit irrationnel.
...
\begin{proof}
    Le fait que, si \( n \) est un carré parfait, alors \( \sqrt{n} \) est rationnel, est évident. Étudions donc la réciproque.
...
Le théorème~\ref{THOooYXJIooWcbnbm} nous a affirmé que tous les \( \sqrt{n}\) sont irrationnels dès que \( n\) n'est pas un carré parfait. Pour \( n=2\),  nous allons en donner une preuve plus explicite. Le fait que \( \sqrt{ 2 }\) existe dans \( \eR\) sera la proposition \ref{PROPooUHKFooVKmpte}.
...
\begin{lemma}[Série géométrique, voir aussi l'exemple~\ref{ExZMhWtJS}]      \label{LEMooOTVUooImvusn}
    Si \( q\in \eQ\) est différent de \( 1 \) et \( p\in \eN\), nous avons
    \begin{equation}
