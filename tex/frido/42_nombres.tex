...
Comme \( (x_n) \) est de Cauchy, il existe \( n_0\) tel que \( | x_p-x_q |< \alpha^2\epsilon / 4 \) pour tout \( p,q>n_0\). Posons \( m_0 = \max \{ n_0 - k_0; 1 \} \). Soit \( m,k\geq m_0\); en posant pour simplifier \( p = m + k_0 \) et \( q = k + k_0 \), nous avons \( |  x_p|>\frac{ \alpha }{2}\) et \( | x_q |>\frac{ \alpha }{2}\), si bien que
            \begin{equation}
                | y_m - y_k | \left| \frac{1}{ x_p }-\frac{1}{ x_q } \right| \leq \frac{ | x_q-x_p | }{ | x_px_q | }\leq | x_q-x_p |\frac 1{| x_p|} \frac 1 {| x_q|} \leq \frac{ \alpha^2 \epsilon }{ 4 } \frac 2 \alpha \frac 2 \alpha \leq \epsilon.
            \end{equation}
...
%---------------------------------------------------------------------------------------------------------------------------
Comme \( \eQ \) est un corps archimédien et donc totalement ordonné, les propriétés sur les suites de Cauchy que l'on a démontrées restent vraies dans \( \eQ\). Récrivons-les dans ce cadre.
