	...
	\begin{equation}
		\begin{aligned}
 			f'(k+1) &= g'(f'(k)) &&\text{(définition)}\\
   					&= g_k\big(f(0), \ldots, f(k-1), f(k)\big)\\
...
\begin{proof}
	...
	\begin{subproof}
		...
		\begin{subproof}
			...
			Nous avons prouvé que \( f(M+1)\leq a\) et \( f(M+1)\geq a\). Nous en déduisons, par antisymétrie de l'ordre (proposition \ref{PROPooVXBBooZcghrA}\ref{ITEMooSoustractionAntisym}), que \( f(M+1)=a\).
		\end{subproof}
	\end{subproof}
\end{proof}
