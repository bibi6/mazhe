...
\begin{proof}
	...
	\begin{subproof}
		...
		%-----------------------------------------------------------
		Montrons que \( f_0(a)=a\) pour tout \( a\in \eN\). Pour cela nous utilisons la récurrence de la proposition \ref{PROPooXTRCooKwrWkq} en posant
		\begin{equation}
			\begin{aligned}
				P\colon \eN & \to \{ 0,1 \}                   \\
				n           & \mapsto \begin{cases}
...
\begin{propositionDef}	\label{PROPooDEFEntiersRelatifsSoustraction}
    ...
    est bien définie, et son image est \( \eZ = Z \setminus\{(-,0)\} \).
 	...
 	Les nombres \defe{entiers}{entier}, appelés aussi \defe{entiers relatifs}{entier!entier relatif}, sont les éléments de \( \eZ \).
\end{propositionDef}
...
\begin{proof}
	Il y a deux choses à prouver. Montrons d'abord que les ensembles donnés sont bien des classes d'équivalence pour \( \sim \). Fixons \( k \in \eN \). Pour tout \( n \in \eN \), on a \( r(n, k+n) = (+,k) \), et pour \( k \neq 0 \), \( r(k+n, n) = (-, k) \); en effet, le seul entier naturel \( x \) tel que \( n + x = n+k \) est \( x = k = (n+k) - n\). Par suite, en rappelant l'ensemble \( Z = \{ +; - \} \times \eN \), on a pour tout \( z \in  Z \), tous les éléments de l'ensemble \( E_z \) ont la même image par \( r \); à l'exception de \( z = (-, 0) \), cette image est \(z \); et comme \( E_{(-,0)} = E_{(+,0)} \), on a \( r(E_{(-,0)}) = \{(+,0)\} \).

  	Soit \( a, b \in \eN \); montrons que la classe d'équivalence dans laquelle est \( (a,b) \) est \( E_{r(a,b)} \). Quand \( a \leq b \), on a \( r(a,b) = (+,b-a) = r(0, b-a) \); en posant \( k = b - a \), et en se rappelant que, pour tout \( n \), on a \( (n + k) - n = k = b - a \) grâce à la proposition \ref{PROPooVXBBooZcghrA}\ref{ITEMooSoustractionNiii}, il s'ensuit que \( r(n, n+k) = r(a,b) \): ainsi, la classe d'équivalence de \( (a,b) \) est formée de tous les couples de la forme \( (n, n+(b-a) \) avec \( n \in \eN\), c'est bien \( E_{r(a,b)} \). Si \( a > b \), on a  \( r(a,b) = (-,a-b) = r(a-b, 0) = r(n+(a-b), n) \) pour tout \( n \), et donc la classe d'équivalence de \( (a,b) \) est aussi \( E_{r(a,b)} \).
\end{proof}

\begin{normaltext}	\label{CORooIntroSommeRelatifs}
...
\begin{propositionDef}[\cite{RWWJooJdjxEK}]	\label{PROPooDEFAdditionEntiersRelatifs}
	Soient \( a, b \in \eZ\). L'application
	\begin{equation}
		\begin{aligned}
...
\begin{proof}
	Réécrite autrement, la proposition \ref{PROPooFIKUooVHlvTt} dit ceci: si \( (x,y) \) et \( (x', y') \) sont dans \( E_a \), si \( (m,p) \) et \( (m',p')\) sont dans \( E_b \), alors \( (x+m,y+p) \sim (x'+p',y'+m') \), c'est-à-dire que les couples \( (x+m,y+p) \) et \( (x'+p',y'+m') \) appartiennent à la même classe d'équivalence que l'on note \( E_z \). Ainsi, l'ensemble image de \( f \) est incluse dans \( E_z \), et par ailleurs \( z = r(x+m,y+p) \).
\end{proof}
...
\begin{proof}
	...
	\begin{subproof}
		\spitem[Neutre]
		L'ensemble \( E_{+0} \) contient exactement les couples de la forme \( (n,n) \) avec \( n \in \eN \). Pour tous entiers naturels \( a,\ b \), on a \( (a+n,b+n) \in E_{r(a,b)} \), donc, pour tout \( z \in \eZ \),
		\begin{equation}
			z + (+0) = z.
		\end{equation}
		De même \( (+0) + z = z \) par commutativité de la somme dans \( \eN\).
		\spitem[Inverse]
