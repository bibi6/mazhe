...
\begin{propositionDef}	\label{PROPooDEFEntiersRelatifsSoustraction}
	On note \( Z = \{ +; - \} \times \eN \). L'application
 	\begin{equation}
		...
		\end{aligned}
  	\end{equation}
    est bien définie, et son image est \( Z \setminus\{(-,0)\} \).
\end{propositionDef}
...
\begin{proof}
	...
	\begin{subproof}
		\spitem[Relation d'équivalence]
  		La fait que \( \sim \) soit réflexive est immédiat. Le fait qu'elle soit symétrique est exactement la commutativité de la somme dans \( \eN\), proposition \ref{PROPooTLTSooGNMTmV}\ref{ITEMooIFFPooXfftfG}. Pour la transitivité, nous supposons que \( (a,b)\sim(u,v)\) et que \( (u,v)\sim(x,y)\). Alors nous avons
		\begin{subequations}
			\begin{align}
				a+v & =u+b  \\
				u+y & =v+x.
...
\begin{lemma}	\label{LEMooEntiersEquivalents}
...
\begin{proposition}	\label{PROPooEquivalenceSoustraction}
  	Soient \( a, b, a', b' \in \eN\). On a \( (a,b)\sim(a',b')\) si et seulement si \( r(a,b) = r(a',b') \), où la fonction \( r \) est définie en \ref{PROPooDEFEntiersRelatifsSoustraction}.
\end{proposition}

\begin{proof}
	Dire que \( (a,b)\sim (a',b')\) est équivalent à dire que \( a+b'=b+a'\). Pour le sens direct, on étudie selon que  \( a \leq b \) ou non.
 	\begin{subproof}
  		\spitem[Si \( a \leq b \):]
  	...
  	Voyons maintenant la réciproque: supposons que \( r(a,b) = r(a',b') \), et étudions de nouveau selon que \( a \leq b \) ou non.
    \begin{subproof}
  		\spitem[Si \( a \leq b \):]
		par définition,  \( r(a,b) = (+, b-a) = r(a', b')  \); puisque la première composante est \( + \), on déduit que \( a' \leq b' \), et donc \( b-a = b' - a' \). Alors \( a + (b' - a') = b \); puis en ajoutant \( a' \) aux deux membres, \( a + (b' - a') + a' = a+b' = b + a' \).
  		\spitem[Sinon:]
