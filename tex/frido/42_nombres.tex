...
\begin{proposition}[\cite{MonCerveau}]	\label{PROPooCONJooTpJwBe}
	Soient deux ensembles \( E\) et \( A\). Nous considérons une application \(g \colon E\to A  \) et nous définissons une relation d'équivalence \( \sim\) sur \( E\) par 
	\begin{equation}	\label{EQNooDefEquivMemeImage}
		 a\sim b \text{\quad si } g(a)=g(b).
	\end{equation}
...
\begin{proof}
	...
	\begin{subproof}
		...
		\spitem[Construction de la fonction \( f \)]
...
\begin{normaltext}		\label{NORMooPUOAooGnFjaX}
	Il est cependant compliqué d'utiliser le théorème de récurrence pour faire une récurrence sur le cardinal d'un ensemble. Supposons que vous vouliez démontrer qu'une propriété est vraie pour tout ensemble fini \( I\). Vu que l'ensemble des ensembles finis n'existe pas, vous ne pouvez pas définir l'application \( P\) qui à un ensemble fait correspondre \( 0\) ou \( 1\).

	Nous pouvons par contre dire \( P(n)=1\) si la propriété est vraie pour tout ensemble de cardinal \( n\). C'est cela que nous entendrons à quelque fois que nous dirons faire une preuve par récurrence sur le cardinal d'un ensemble.
\end{normaltext}

\begin{normaltext}[\cite{MonCerveau}]      \label{NORMooLNXMooWIblPf}
	Le théorème \ref{THOooEJPYooZFVnez} nous permet de définir une suite \( (x_n)_{n\in \eN}\) par récurrence en posant par exemple\footnote{Les spépieu\randomGender{x}{ses} diront que \( + \) n'est pas encore défini, et ils auront raison. Qu'ils remplacent les \( +1 \) par des applications de \( s \) et les \( +2 \) par des double-applications de \( s \): tout fonctionne très bien.}
	\begin{subequations}
		\begin{numcases}{}
			x_0=b\\
			x_{n+1}=f(x_n).
...
\begin{definition}      \label{DEFooLCWLooYrToFv}
	Soit \( (A,+, \leq)\) un ensemble totalement ordonné muni d'une loi de composition interne, et admettant un neutre \( e \). On dit que \( A \) est \defe{archimédien}{ensemble!archimédien}\index{archimédien} si pour tout \( x,y\in A\) avec \( x>0\), il existe \( n\in \eN\) tel que \( \phi_e(x,n) \geq y\) (avec \( phi_e \) donné par le corolaire \ref{CORooItereesOperation}).
\end{definition}
...
\begin{propositionDef}[\cite{RWWJooJdjxEK,MonCerveau}]      \label{PROPooVFOXooXmwpFh}
	...
	\end{enumerate}
	Pour \( a,b\in \eN\) nous notons \( f(a,b)=a+b\) et nous appelons cela la \defe{somme}{entiers naturels!somme}\index{somme!entiers naturels} de \( a \) et \( b \)
\end{propositionDef}
...
\begin{propositionDef}	\label{PROPooDEFSoustractionN}
	Soit \( a,b\in \eN\), tels que \( a\leq b\). L'élément  \( x\in \eN\) tel que \( a+x=b\) est unique, on note \( x = b - a \), et on dit que \( x \) est la \defe{différence}{différence!entiers naturels} entre \( b \) et \( a \)  (ou la \defe{soustraction}{soustraction!dans \( \eN \)} de \( a \) à \( b \)).
\end{propositionDef}
...
\begin{proof}
	...
	\begin{subproof}
		...
		\spitem[Point \ref{ITEMooSoustractionNii} : transitivité]
		...
		\spitem[Point \ref{ITEMooSoustractionNiii} : compatibilité]
...
% TODOooPropsSoustractionN sans doute faut-il mettre ici les autres propriétés relatives à la soustraction dans N.
...
\begin{definition}      \label{DEFooKBUFooLvMHrf}
	...
	\end{equation}
 	Il s'agit d'un intervalle\footnote{Les intervalles ont été définis de manière générale en la définition\ref{DefEYAooMYYTz}.} dans \( \eN \). \index{intervalle!d'entiers naturels}
\end{definition}
...
\begin{propositionDef}      \label{PROPooBBQPooRgPOjf}
	...
	\end{enumerate}
	Cette fonction est la \defe{multiplication}{multiplication!entiers naturels} et nous notons \( \phi (a,b)=a\times b\), voire \( ab\) quand il n'y a pas d'ambigüité. Le nombre \( a\times b\) est nommé le \defe{produit}{produit} de \( a\) par \( b\).
\end{propositionDef}
...
\begin{proof}
	Point par point, avec beaucoup de récurrence\footnote{Nous les détaillerons très peu, mais nous invitons \randomGender{le lecteur}{la lectrice} à compléter et appeler correctement le théorème \ref{THOooEJPYooZFVnez}.}
	\begin{subproof}
...
\begin{propositionDef}[Puissances entières]	\label{PROPooDEFPuissanceEntiers}
	...
	\end{enumerate}
Cette fonction est la \defe{fonction puissance}{puissance!entiers naturels} et nous notons \( \phi (a,b)=a^b\).
\end{propositionDef}
...
\begin{remark}	\label{REMooProprietesPuissance}
...
%---------------------------------------------------------------------------------------
\subsection{Construction de l'ensemble}
...
\begin{propositionDef}	\label{PROPooDEFEntiersRelatifsSoustraction}
    ...
    est bien définie, et son image est \( \eZ = Z \setminus\{(-,0)\} \).
 	...
 	Les nombres \defe{entiers}{entier}, appelés aussi \defe{entiers relatifs}{entier!entier relatif} \index{entiers relatifs}, sont les éléments de \( \eZ \).
\end{propositionDef}
...
\begin{definition}	\label{DEFooEntiersRelatifsSigneDistance}
	Soit \( a \in \eZ \). On a soit \( a = (+,k) \) ou \( a = (-, k) \) pour un certain \( k \in \eN \).
 	...
 	Le \defe{signe}{entiers relatifs!signe} de \( a \) est le symbole \( + \) ou \( - \) apparaissant en première composante. On dit que \( a \) est \defe{positif}{entiers relatifs!positifs} si le signe est \( + \), \defe{négatif}{entiers relatifs!négatifs} sinon.

  	La \defe{distance à zéro}{entiers relatifs!distance à zéro} de \( a \) est le nombre \( k \) apparaissant en deuxième composante. C'est aussi la \defe{valeur absolue}{valeur absolue!entiers relatifs} de \( a \).
\end{definition}

\begin{normaltext}	\label{NORMooEntiersRelatifsEquivalence}
	La fonction \( r \) nous permet d'obtenir une relation d'équivalence sur \( \eN \times \eN \) (qu'on notera dès à présent \( \eN^2 \) ). On \og copie \fg\ l'exemple \ref{EXooProjCanoniqueEquivAppl} %besoin de PROPooCONJooTpJwBe ?
...
\begin{proposition}[\cite{RWWJooJdjxEK}]     \label{PROPooFIKUooVHlvTt}
...
\begin{proposition}	\label{PROPooClassesEquivalenceRelatifs}
...
%---------------------------------------------------------------------------------------------------------------------------
\subsection{Somme d'entiers relatifs}
	...
	Soit \( m, n \in \eN \). L'addition dans \( \eZ \) obéit aux règles suivantes:
 	\begin{multicols}2
  	\begin{align}
		...
		\text{si } m, n \neq 0,\quad (-m) + (-n) &= (-(m+n))	\label{SEQooAdditionRelatifsMM}\\
		\text{si }0 < m \leq n,\quad (-m) + (+n) &= (+(n-m))	\label{SEQooAdditionRelatifsMPP}\\
  		\text{si }m < n,\quad (+m) + (-n) &= (-(n-m))	\label{SEQooAdditionRelatifsPMM}\\
		\text{si }0 < n < m,\quad (-m) + (+n) &= (-(m-n))	\label{SEQooAdditionRelatifsMPM}\\
  		\text{si }m \geq n, \quad (+m) + (-n) &= (+(m-n)).	\label{SEQooAdditionRelatifsPMP}
  	\end{align}
   \end{multicols}
\end{corollary}
...
\begin{propositionDef}	\label{PROPooGroupeEntiersRelatifs}
	L'ensemble \( \eZ \), muni de l'addition telle que définie par la proposition \ref{PROPooDEFAdditionEntiersRelatifs}, est un groupe commutatif:
 	\begin{itemize}
  	...
  	\end{itemize}
    Pour tout \( z \in \eZ \), on notera \( -z \) l'inverse de \( z \), qu'on appellera \defe{opposé}{entier!opposé} de \( z \).
\end{propositionDef}

\begin{proof}
	En plusieurs points.
	\begin{subproof}
		\spitem[Commutativité, Neutre]
...
\begin{theoremDef}[Résolution d'équations additives dans \( \eZ \) ]	\label{THMooEquationsAdditivesEntiersRelatifs}
	Soit \( a, b \in \eZ \). Il existe un unique \( x \in \eZ\) tel que \( a+x = b \). Cet entier est \( x = b + (-a) \), noté plus simplement \( x = b - a \), et c'est la \defe{différence}{différence!entiers relatifs} des entiers \( b \) et \( a \).
\end{theoremDef}
...
\begin{proposition}[Relation d'ordre \cite{RWWJooJdjxEK}]       \label{PROPooMYYDooOABOdB}
...
\begin{normaltext}	\label{NORMooOrdreEntiersRelatifsExtension}
	% TODOooProblemeLabel Ce label me semble dangereux parce que si on le cite dans une démonstration entre ici et LEMooKAXFooIPyzJC, le détecteur de référence vers le futur
	% ne préviendra pas du problème. Bibi se demande pourquoi... mais a besoin de tous ces labels.
...
%TODOooPreuveMultiplicationSigne
