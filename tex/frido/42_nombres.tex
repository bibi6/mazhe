...
\begin{lemma}[Propriétés de la valeur absolue]  \label{LemooANTJooYxQZDw}
    ...
    \begin{enumerate}
        ...
        \item\label{ItemooOMKNooRlanvki}
          pour tout entier naturel \(n\), \( |nx| \leq n |x| \); et pour tout entier \( n \),  \( |nx| \leq |n| |x| \). \quext{Ça serait génial si on avait l'égalité, mais a priori je ne vois pas pourquoi on l'aurait. Contre-exemple, ou preuve?}
    \end{enumerate}
\end{lemma}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        \item
            ...
            \end{enumerate}
            \item
              Se démontre par récurrence à l'aide de la propriété précédente.
    \end{enumerate}
\end{proof}

%TODO: preuve par récurrence à faire.
...
\begin{proposition}\label{PROPCauchyKdoncBorne}
...
\begin{proof}
  Soit \( (x_n)\) une suite de Cauchy dans \( \eK\). Avec \( \epsilon=1\) dans la définition~\ref{DefKCGBooLRNdJf}~\ref{ItemVXOZooTYpcYN}, si \( n>N_1\), nous avons
            \begin{equation}
                | x_n-x_{N_1} |\leq 1.
            \end{equation}
            Et donc pour tout \( n\) plus grand que \( N_1\), \( x_N-1\leq x_q\leq x_N+1\), ou encore, pour tout \( n  \in \eN \) :
            \begin{equation}
...
\begin{proposition}[Suites de Cauchy et opérations]\label{PROPCauchyKetOperations}
   Soient \( (x_n)\) et \( (y_n)\) deux suites de Cauchy dans un corps totalement ordonné \( \eK \). Alors les suites \( (x_n+y_n)\) et \( (x_n-y_n)\) sont de Cauchy.
   Si de plus \( \eK \) est archimédien, alors \( (x_ny_n)\) est également de Cauchy.
\end{proposition}
...
\begin{proof}
  ...
  Passons à la démonstration du fait que le produit de deux suites de Cauchy est de Cauchy. Soit \( \epsilon > 0 \). Pour tous \(p, q \in \eN \), on a:
            \begin{equation}
                | x_py_p-x_qy_q |\leq | x_py_p-x_qy_p |+| x_qy_p-x_qy_q |\leq | y_p (x_p-x_q) |+| x_q(y_p-y_q) | \leq \bigl| y_p |x_p-x_q| \bigr|+ \bigl| x_q |x_p-x_q| \bigr|.
            \end{equation}
            Par la proposition~\ref{PROPCauchyKdoncBornee}, les suites \( (x_n)\) et \( (y_n)\) sont bornées et quitte à prendre le maximum, puis l'entier qui le dépasse\footnote{$\eK$ est supposé archimédien.}, nous disons qu'elles sont toutes les deux bornées par l'entier naturel \( M\) : pour tout \( n\) nous avons \( | x_n |\leq M\) et \( | y_n |\leq M\). Par ailleurs, \( (x_n)\) et \( (y_n)\) sont de Cauchy, si \( p\) et \( q\) sont assez grands (disons plus grands qu'un entier naturel \( N \)), les deux différences sont majorées par \( \epsilon / 2M\). Ainsi, par le lemme~\ref{LemooANTJooYxQZDw}~\ref{ItemooOMKNooRlanvki}, nous obtenons, pour \( p, q > N \):
            \begin{align}
              | x_py_p-x_qy_q |%
              & \leq\bigl| y_p |x_p-x_q| \bigr|+ \bigl| x_q |x_p-x_q| \bigr|\\
              & \leq\bigl| M |x_p-x_q| \bigr|+ \bigl| M |x_p-x_q| \bigr|\\
...
\begin{proposition}[Convergence de suites et opérations]
   ...
   Alors les suites \( (x_n+y_n)\) et \( (x_n-y_n)\) convergent respectivement vers \( a+b \) et \( a - b \).
   Si de plus \( \eK \) est archimédien, alors \( (x_ny_n)\) converge vers \( ab\).
\end{proposition}
...
\begin{proof}
        En ce qui concerne la somme, nous pouvons tout de suite calculer
            ...
            \begin{equation}
                | x_ny_n-ab |\leq | x_ny_n-ay_n |+| ay_n-ab |\leq \bigl| y_n | x_n-a |\bigr|+\bigl| a | y_n-b |\bigr|.
            \end{equation}
            La suite \( (y_n)\) est bornée parce que convergente (corollaire~\ref{CorSuiteConvergenteKBornee}). Comme \( \eK \) est archimédien, posons \( M \) un entier naturel qui majore \(|a|\) et tous les éléments de cette suite. Par  le lemme~\ref{LemooANTJooYxQZDw}~\ref{ItemooOMKNooRlanvki}, nous avons
            \begin{equation}
                | x_ny_n-ab |\leq  \bigl| M | x_n-a |\bigr|+\bigl| M | y_n-b |\bigr| \leq M | x_n-a | + M | y_n-b |.
            \end{equation}
...
\begin{proposition}\label{PropSuiteDInverses}
            Soit \( (x_n)\) une suite de Cauchy dans le corps totalement ordonnéet archimédien \(\eK\), qui ne converge pas vers zéro\footnote{Elle peut donc converger vers autre chose que 0, voire même ne pas converger dans $\eK$.}. Alors il existe \( k_0 \in \eN \) tel que la suite \( (y_n) \) définie par \( y_n = \frac{1}{ x_{n+k_0} } \) soit de Cauchy.
\end{proposition}
...
Comme \( \eK \) est archimédien, il existe un naturel \( M \) tel que \( 2 /\alpha < M \). Comme \( (x_n) \) est de Cauchy, il existe \( n_0\) tel que \( | x_p-x_q |< \epsilon / M^2 \) pour tout \( p,q>n_0\). Posons \( m_0 = \max \{ n_0 - k_0; 1 \} \). Soit \( m,k\geq m_0\); en posant pour simplifier \( p = m + k_0 \) et \( q = k + k_0 \), nous avons \( |  x_p|>\frac{ \alpha }{2}\) et \( | x_q |>\frac{ \alpha }{2}\). Il s'ensuit que \( 1/ |  x_p| < M \) et que  \( 1/ |  x_q| < M \), si bien que
\begin{align}
  | y_m - y_k | &= \left| \frac{1}{ x_p }-\frac{1}{ x_q } \right|\\
                &= \left|\frac{  x_q-x_p  }{  x_px_q }\right|\\
                &\leq \left| |x_q-x_p| \frac 1{| x_p|} \frac 1 {| x_q|}\right|\\
                & \leq \left| |x_q-x_p| M^2\right| \\
                &\leq M^2 \frac \epsilon{M^2}
                &\leq \epsilon.
\end{align}
