...
\begin{definition}  \label{EQooYCWSooIhibvd}
  ...
  Dans $\eR$, les boules sont les intervalles ouverts et fermés tandis que la sphère est donnée par les points extrêmes des intervalles :
	\begin{equation}
		\begin{aligned}
			...
			\bar B(a,r)&=\mathopen[ a-r , a+r \mathclose],\\
			S(a,r)&=\{ a-r,a+r \}.
		\end{aligned}
	\end{equation}
        Tout cela reste cohérent avec les boules que l'on a définies pour les corps ordonnés (définitions \ref{DefKCGBooLRNdJf}).
\end{example}
Nous prouverons plus tard (proposition~\ref{PropBoulesOuvertesSontOuverts}) que les noms donnés aux boules sont cohérents avec ce que l'on attend. Mais voyons d'abord la topologie que l'on obtient avec les espaces métriques.
\begin{theoremDef}     \label{ThoORdLYUu}
    Soit \( (E,d)\) un espace métrique. Alors en posant
...
\begin{remark}
  Cette définition est faite exprès pour respecter le théorème~\ref{ThoPartieOUvpartouv}. Même si, a priori, on aurait dû utiliser la topologie engendrée faite à l'exemple \ref{DefTopologieEngendree}\dots\ mais on peut montrer que les deux topologies sont les mêmes. C'est un peu le but de la proposition suivante.
\end{remark}

\begin{proposition}\label{PropBoulesBaseTopoMetrique}\label{ITEMooUIHJooXAFaIz}
      Les boules ouvertes sont une base de la topologie (définition~\ref{DefQELfbBEyiB}) des espaces métriques.
\end{proposition}

\begin{proof}
  Nous allons montrer que dans une intersection finie non-vide de boules ouvertes, on trouve encore une boule ouverte. Soit \( n \in \eN \), et pour \(i = 1,\ldots\ , n \), \(x_i \in X\) les centres des boules et \( r_i \) leurs rayons. Il existe \( z \in X \) tel que pour tout \(i,\ d(x_i,z) < r_i \).

  Notons alors \( \epsilon_i = 1 - \frac  { d(x_i,z)} {r_i} \), puis \(\epsilon = \min \epsilon_i \), et posons \( r = \epsilon \min r_i \). Montrons que \(B(z,r) \) est incluse à toutes les boules.

  Soit \( y \in B(z,r)\). Pour tout \(i = 1,\ldots\ , n\), on a:
  \begin{align}
    d(x_i,y)&\leq d(x_i,z) + d(z,y)\\
            &\leq (1 - \epsilon_i) r_i + r\\
            &\leq (1 - \epsilon_i) r_i + \epsilon r_i\\
            &\leq \bigl((1 - \epsilon_i) + \epsilon_i\bigr) r_i\\
            &\leq r_i;
  \end{align}
  donc \(y \in B(x_i, r_i)\).
\end{proof}

\begin{corollary}\label{CORooUIHJooXAFaJa}\label{ITEMooUIHJooXAFaJa}
...
\begin{normaltext}
    ...
    De plus, les boules définies en~\ref{DefKCGBooLRNdJf} sont alors les mêmes que celles définies en \eqref{EQooYCWSooIhibvd}, ce qui donne à tout corps totalement ordonné une structure d'espace métrique.
\end{normaltext}

\begin{proposition}
  Les boules ouvertes définies en \ref{EQooYCWSooIhibvd} sont ouvertes pour la topologie métrique.

  De même, les boules fermées sont fermées pour la topologie métrique.
\end{proposition}

\begin{proof}
  D'abord, \( \Int\bigl( B(a,r) \bigr)=B(a,r)\). En effet, si $x\in B(a,r)$, nous avons $d(a,x)<r$, et la boule $B\big(x,r-d(x,a)\big)$ est incluse à $B(a,r)$. Ainsi, $x$ est dans l'intérieur de $B(a,r)$. Conseil : faire un dessin.

  Ensuite, \( \Adh \bigl(\bar B(a,r)\bigr) = \bar B(a,r)\). Si \( y \) est dans l'adhérence de la boule fermée, alors, pour tout \(\epsilon > 0\), la boule \( B(y, \epsilon) \) intersecte \(\bar B(a,r)\): il existe \( z_\epsilon \) tel que \( d(y,z_\epsilon) < \epsilon \) et \( d(a, z_\epsilon) < r \). Il s'ensuit que, pour tout \(\epsilon > 0\),
  \begin{equation}
    d(y,a) \leq d(y,z_\epsilon) + d(z_\epsilon, a) \leq r + \epsilon;
  \end{equation}
  donc, nécessairement, \( d(y,a) \leq r \), soit \( y \in  \bar B(a,r) \).
\end{proof}

\begin{remark}
  On pourrait penser que l'intérieur d'une boule fermée est la «même» boule ouverte\dots{} mais ce n'est pas vrai en général. Voici un contre-exemple.

  Sur \( \eR \), on considère la distance:
  \begin{equation}
    d(x,y) =
    \begin{cases}
      0 &\text{si }x-y \in \eQ\\
    ...
    \end{cases}
  \end{equation}
  Les ouverts pour la topologie générée est constituée d'unions de \( \eQ \) et de ses classes latérales. Considérons alors la boule ouverte \( B(0,1) \): c'est précisément \( \eQ \). La boule fermée \( \bar B(0,1) \) est en fait \( \eR \) tout entier\dots\ dont l'intérieur est lui-même, et non \( \eQ \).

  Cela est lié au fait que \( S(0,1) = \eR \setminus \eQ \) est un ouvert pour cette topologie.
\end{remark}
...
\begin{proof}
    ...
    Soit \( a\in A\) avec \( d(a,x) <\frac{ r }{ 3 }\) (existe par densité de \( A\) dans \( E\)); nous avons \( B(a,\frac{ 2r }{ 3 })\subset B(x,r)\) parce que si \( y\in B( a,\frac{ 2r }{ 3 } )\) alors
    \begin{equation}
        d( y, x) \leq d(y, a) + d( a, x) < \frac{ 2 }{ 3 }r+\frac{ 1 }{ 3 }r=r.
    \end{equation}
...
\begin{remark}      \label{RemIPVLooHUXyeW}
    De par la construction faite à la définition~\ref{DefTopologieRn}, il est vite vu que les cubes ouverts forment une base de la topologie de \( \eR^n\). Mais, sur \( \eR^n \), on a aussi la distance «usuelle» euclidienne. Cela est à mettre en rapport avec le fait que toutes les normes sont équivalentes sur \( \eR^n\) (proposition~\ref{ThoNormesEquiv}).
...
\begin{proposition}[Continuité, ouverts et voisinages et limite\cite{DHpsZoY}] \label{PropQZRNpMn}
    ...
    \begin{enumerate}
        ...
        \item\label{ItemYNQpikriii}
            Pour tout \( \epsilon>0\), il existe \( \delta>0\) tel que, pour tout \( x \in E\), si \( d_E(x,a)<\delta\), alors \( d_F\bigl(f(x),f(a)\bigr)<\epsilon\).
    \end{enumerate}
\end{proposition}
...
\begin{proof}
    L'équivalence~\ref{ItemCBUoRWJi} \( \Leftrightarrow\)~\ref{ItemCBUoRWJii} est la définition~\ref{DefOLNtrxB}. L'équivalence~\ref{ItemCBUoRWJi} \( \Leftrightarrow\)~\ref{ItemYNQpikrii} est la définition~\ref{DefOLNtrxB} de la continuité en un point couplée à l'unicité de la limite due à la proposition~\ref{PropFObayrf} parce qu'un espace métrique est séparé (remarque~\ref{RemQDRooKnwKk}). Ensuite, l'équivalence~\ref{ItemCBUoRWJiii} \( \Leftrightarrow\)~\ref{ItemCBUoRWJiv} est une simple paraphrase.
    ...
    Prouvons~\ref{ItemCBUoRWJiii} \( \Rightarrow\)~\ref{ItemYNQpikriii}. Soient \( \epsilon>0\); posons \( W=B\big( f(a),\epsilon \big)\). Il existe un voisinage \( V\) de \( a\), de la forme \( V = B(a, \delta) \), tel que \( f(V)\subset W\). Pour ce \( \delta \), si \( d_E(x,a) <\delta\) nous avons \( x\in B(x,\delta)\subset W\) et donc \( f(x)\in B\big( f(a),\epsilon \big)\), c'est à dire \( d_F\bigl(f(x),f(a)\bigr)<\epsilon\).

    Terminons en prouvant \ref{ItemYNQpikriii} \( \Rightarrow\)~\ref{ItemCBUoRWJiv}. Soit \( \epsilon > 0 \), on sait par \ref{ItemYNQpikriii} qu'il existe \( \delta > 0 \) tel que, en paraphrasant, si \( x \in E \) est tel que \( x \in B(a,\delta)\), alors \( f(x) \in B(f(a),\epsilon)\). Mais alors, les images de tous les points de \(B(a,\delta)\) par \( f \) sont dans \( B(f(a),\epsilon)\), en d'autres termes: \( f\big( B(a,\delta) \big)\subset B\big( f(a),\epsilon \big) \).
\end{proof}
...
\begin{lemma}[de Lebesgue\cite{AntoniniAndAl-EspacesMetriquesCompacts}]    \label{LemQFXOWyx}
    Soit \( (X,d)\) un espace métrique séquentiellement compact\footnote{Définition~\ref{DefEspaceSeqCompact}.}. Si \( \{ V_i \}\) est un recouvrement par des ouverts de \( X\), alors il existe \( \epsilon\) tel que pour tout \( x\in X\), nous ayons \( B(x,\epsilon)\subset V_i\) pour un certain \( i\).
\end{lemma}
...
\begin{lemma}[\cite{AntoniniAndAl-EspacesMetriquesCompacts}]   \label{LemMGQqgDG}
    Soit \( (X,d)\) un espace métrique séquentiellement compact\footnote{Définition~\ref{DefEspaceSeqCompact}.}. Pour tout \( \epsilon>0\), il existe un ensemble fini \( \{ x_i \}_{i\in I}\) tel que les boules \( B(x_i,\epsilon)\) recouvrent \( X\).
\end{lemma}
...
\begin{theorem}[Bolzano-Weierstrass\cite{AntoniniAndAl-EspacesMetriquesCompacts}, thème \ref{THEMEooQQBHooLcqoKB}]\label{ThoBWFTXAZNH}
    Un espace métrique est compact\footnote{Définition~\ref{DefJJVsEqs}.} si et seulement s'il séquentiellement compact\footnote{Définition~\ref{DefEspaceSeqCompact}.}.
\end{theorem}
...
\begin{remark}
  La valeur absolue est finalement la même que celle du corps ordonné \( \eR \) (définition~\ref{DefKCGBooLRNdJf}). Avec la proposition~\ref{TopologieR_ord_top}, on constate que, finalement, on retombe sur la topologie usuelle donnée avec la définition~\ref{DefTopologieREngendree}.
\end{remark}
...
\begin{proposition}		\label{PropTopologieAx}
	...
	\begin{enumerate}
		\item
			L'ensemble $V$ lui-même et le vide sont à la fois fermés et ouverts, et ce sont les seuls à avoir cette propriété.
		\item
	...
	\begin{enumerate}
		...
		\item
			Nous avons déjà dit que, par définition, l'ensemble vide est ouvert. Cela implique que $V$ lui-même est fermé (parce que son complémentaire est le vide). De plus, $V$ est ouvert parce que toutes les boules sont inclues à $V$. Le vide est alors fermé (parce que son complémentaire est $V$).
                        ...
                        Reste à voir que ce sont les seuls.
%TODO: le faire.
		\item
