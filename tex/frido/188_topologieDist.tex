...
\begin{remark}  \label{RemQDRooKnwKk}
    ...
    \item      \label{ITEMooUIHJooXAFaIz}
      Les boules ouvertes sont une base de la topologie (définition~\ref{DefQELfbBEyiB}) des espaces métriques. En effet, dans une intersection finie de boules ouvertes, on trouve encore une boule ouverte.
...
\begin{proof}
  Du corps ordonné \( \eR \), on recrée les intervalles, et donc la topologie. Reste à voir le sens inverse: est-ce que, de tous les intervalles, on recrée les boules? La réponse est oui: de l'intervalle \( \mathopen]a, b\mathclose[ \), on obtient la boule \( B(\frac{a+b} 2, \frac{b-a} 2) \).
\end{proof}
